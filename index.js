...
async function getSpecificComment(prompt, message) {
    const [rows] = await pool.query(`SELECT easteregg_content AS easterEggContent FROM EasterEgg WHERE 1=1AND bot_type = 'cold-jimin' AND INSTR(easteregg_input, ?) > 0 ORDER BY easteregg_input, rand()`, [prompt]);
    if (rows.length > 0) {
